...
public:
    ...
    void start() {
        ...
        std::cout << "🖥️ Servers:   http://localhost:" << port_ << "/servers" << std::endl;
...
<body>
    ...
    std::string generateServersPage() {
        ...
        html << R"(
        <div class="card">
            <h2>🖥️ Chunk Servers <button class="refresh" onclick="refreshPage()">🔄 Refresh</button></h2>
...
int main() {
    ...
    // Load any existing files to show in web interface
    std::cout << "📁 Loading existing files..." << std::endl;
